...
public class DashboardController {
    ...
    public ResponseEntity<List<DashboardUserProductStatDTO>> getUserProductStatistics(
            @RequestParam(required = false) String start,
            @RequestParam(required = false) String end,
            @RequestParam(defaultValue = "month") String groupBy) {
        return ResponseEntity.ok(dashboardService.getUserProductStatistics(start, end, groupBy));
    }
}
